...
angular.module('imago').run(['$templateCache', function($templateCache) {$templateCache.put('/imago/imago-slider.html','<div ng-class="[imagoslider.conf.animation, action]" ng-swipe-left="imagoslider.goNext($event)" ng-swipe-right="imagoslider.goPrev($event)" class="imago-slider-content"><div ng-show="imagoslider.conf.enablearrows &amp;&amp; (imagoslider.length &gt; 1 || imagoslider.conf.siblings)" ng-click="imagoslider.goPrev($event)" analytics-on="click" analytics-event="Previous Slide" class="prev"></div><div ng-show="imagoslider.conf.enablearrows &amp;&amp; (imagoslider.length &gt; 1 || imagoslider.conf.siblings)" ng-click="imagoslider.goNext($event)" analytics-on="click" analytics-event="Next Slide" class="next"></div></div>');}]);