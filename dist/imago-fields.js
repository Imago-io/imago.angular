    ...
    return {
      ...
      link: function(scope, element, attrs, ngModelController) {
        ...
        scope.$watchCollection('ngModel', function(value) {
          var currency, i, len, ref1;
          if (!_.isPlainObject(scope.ngModel)) {
            return scope.notComplete = true;
          }
...
$templateCache.put("/imago/imago-field-currency.html","<div ng-class=\"{expanded: input.expanded}\" class=\"imago-field currency\"><button ng-click=\"input.expanded = !input.expanded\"></button><div ng-if=\"!input.expanded\" ng-class=\"{focus:onfocus}\" class=\"wrapper compact\"><label><span ng-repeat=\"cur in currencies\" ng-click=\"currency = cur\">{{cur}}</span></label><input type=\"text\" imago-filter-currency=\"imago-filter-currency\" ng-model=\"ngModel[currency]\" ng-model-options=\"{updateOn: \'blur\'}\" ng-change=\"update(ngModel); onfocus = false\" ng-disabled=\"!currency\" ng-focus=\"onfocus = true\"/></div><div ng-repeat=\"cur in currencies\" ng-if=\"input.expanded\" class=\"wrapper expanded\"><div class=\"imago-field\"><label>{{cur}}</label><input type=\"text\" currency=\"{{cur}}\" imago-filter-currency=\"imago-filter-currency\" ng-model=\"ngModel[cur]\" ng-focus=\"onFocus(input)\" ng-blur=\"unbindSave(input); saveOnBlur(input);\"/></div></div><div ng-show=\"notComplete\" class=\"imago-message-info\">somes currencies do not have any valued</div></div>");
